...
<head>
    ...
    <link rel="stylesheet" href="<?php echo htmlspecialchars($_SERVER['SCRIPT_NAME']); ?>assets/css/style.css">
...
<body>
    <header class="header">
        <div class="header-content">
            <div class="logo">
                <a href="<?php echo htmlspecialchars($_SERVER['SCRIPT_NAME']); ?>">
                    <span class="logo-text">Système d'Inscription</span>
                </a>
            </div>
            <nav class="nav">
                <?php if (strpos($_SERVER['REQUEST_URI'], '/admin/') !== false): ?>
                    <a href="<?php echo htmlspecialchars($_SERVER['SCRIPT_NAME']); ?>admin/" class="nav-link">Tableau de bord</a>
                    <a href="<?php echo htmlspecialchars($_SERVER['SCRIPT_NAME']); ?>admin/manage.php" class="nav-link">Gérer les listes</a>
                    <a href="<?php echo htmlspecialchars($_SERVER['SCRIPT_NAME']); ?>admin/import.php" class="nav-link">Importer</a>
                    <a href="<?php echo htmlspecialchars($_SERVER['SCRIPT_NAME']); ?>admin/export.php" class="nav-link">Exporter</a>
                    <a href="<?php echo htmlspecialchars($_SERVER['SCRIPT_NAME']); ?>admin/?logout=1" class="nav-link">Déconnexion</a>
                <?php else: ?>
                    <a href="<?php echo htmlspecialchars($_SERVER['SCRIPT_NAME']); ?>user/" class="nav-link">Accueil</a>
                    <?php if (isset($_SESSION['user_name'])): ?>
                        <a href="<?php echo htmlspecialchars($_SERVER['SCRIPT_NAME']); ?>user/?logout=1" class="nav-link">Changer de nom</a>
                    <?php endif; ?>
                <?php endif; ?>
            </nav>
        </div>
